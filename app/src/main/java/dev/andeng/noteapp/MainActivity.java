...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        saveButton.setOnClickListener(view -> {
            saveNote();
            displayNotes(noteList); // Refresh and sort the notes after saving
        });
    ...
    private void displayNotes(List<Note> notes) {
        System.out.println("Displaying notes. Total notes: " + notes.size()); // Debug statement
        sortNotesByPinStatus(notes); // Sort the notes based on their pin status
    ...
    private void sortNotesByPinStatus(List<Note> notes) {
        Collections.sort(notes, (n1, n2) -> {
            if (n1.isPinned() &&!n2.isPinned()) {
                ...
                return 0; // Both notes have the same pin status
            }
        });
    }
    ...
    private void createNoteView(final Note note) {
        ...
        Button pinButton = noteView.findViewById(R.id.pinButton); // Find the pin button

        titleTextView.setText(note.getTitle());
        contentTextView.setText(note.getContent());

        // Initialize pin button state based on note.isPinned
        pinButton.setText(note.isPinned()? "Unpin" : "Pin");

        pinButton.setOnClickListener(v -> {
            note.setIsPinned(!note.isPinned()); // Correctly toggles the pin status using the setter method
            pinButton.setText(note.isPinned()? "Unpin" : "Pin"); // Corrected to use the getter method
            saveNotesToPreferences(); // Save the updated note
            displayNotes(noteList);
        });
